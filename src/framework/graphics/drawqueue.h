...
struct DrawQueueItemTexturedRect : public DrawQueueItem {
    ...
    bool m_noCache = false;   // skip atlas cache (needed for rotated/shadow draws)
    bool m_isShadow = false;  // draw as black silhouette via color shader
...
// Renders a whole sub-queue into an offscreen framebuffer and then draws that
// framebuffer back on screen through a shader. The sub-queue is built while the
// draw lists are being filled (same thread/order as if it were enqueued
// directly), so no extra synchronization is needed.
// The shader receives the destination rect as u_Center (center, in screen
// pixels) and u_Offset (half size, in screen pixels), which lets it map the
// fragment back to a position inside the widget. u_Resolution holds the screen
// size the framebuffer was rendered at.
struct DrawQueueItemFramebufferWithShader : public DrawQueueItem {
    DrawQueueItemFramebufferWithShader(const std::shared_ptr<DrawQueue>& queue, const Rect& dest, const std::string& shader) :
        DrawQueueItem(nullptr), m_queue(queue), m_dest(dest), m_shader(shader) {}
    virtual ~DrawQueueItemFramebufferWithShader() = default;

    virtual void draw();
    virtual bool cache() { return false; }

    std::shared_ptr<DrawQueue> m_queue;
    Rect m_dest;
    std::string m_shader;
};
...
struct DrawQueueItemNormalMapped : public DrawQueueItem {
    DrawQueueItemNormalMapped(const Rect& dest, const TexturePtr& texture, const Rect& src,
                              const TexturePtr& normalMap, const Color& color)
        : DrawQueueItem(texture, color), m_dest(dest), m_src(src), m_normalMap(normalMap) {}

    void draw() override;
    void draw(const Point& pos) override;
    bool cache() override { return false; } // Normal mapped items can't be atlas-cached
...
public:
    ...
    DrawQueueItemNormalMapped* addNormalMappedRect(const Rect& dest, const TexturePtr& texture, const Rect& src,
                                                      const TexturePtr& normalMap, const Color& color = Color::white)
    {
        DrawQueueItemNormalMapped* item(new DrawQueueItemNormalMapped(dest, texture, src, normalMap, color));
        m_queue.push_back(item);
        return item;
    }
    ...
    void setFlip(size_t start, uint8_t flipDirection)
    {
        for (size_t i = start; i < m_queue.size(); ++i) {
            DrawQueueItemTexturedRect* texture = dynamic_cast<DrawQueueItemTexturedRect*>(m_queue[i]);
            if (texture) {
                texture->m_noCache = true;
                texture->m_flipDirection = flipDirection;
            }
        }
    }
    ...
    void setShadow(size_t start)
    {
        for (size_t i = start; i < m_queue.size(); ++i) {
            DrawQueueItemTexturedRect* texture = dynamic_cast<DrawQueueItemTexturedRect*>(m_queue[i]);
            if (texture) {
                texture->m_noCache = true;
                texture->m_isShadow = true;
            }
        }
    }
    ...
    void scaleOutfit(int fromPos, float scale);
