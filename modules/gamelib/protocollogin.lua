...
-- account + character creation (custom Avatar feature, handled by protocollogin.cpp)
function ProtocolLogin:createAccount(host, port, accountName, accountPassword, charName, sex, vocation)
  if string.len(host) == 0 or port == nil or port == 0 then
    signalcall(self.onLoginError, self, tr("You must enter a valid server address and port."))
    return
  end
  ...
  self.accountName = accountName
  self.accountPassword = accountPassword
  self.createCharName = charName
  self.createSex = sex
  self.createVoc = vocation
  self.connectCallback = self.sendLoginPacket
...
function ProtocolLogin:sendLoginPacket()
  ...
  -- account creation (custom): a character name + sex byte right after the password
  -- turns this login packet into an account-creation request on the server side.
  if self.createCharName and #self.createCharName > 0 then
    msg:addString(self.createCharName)
    msg:addU8(self.createSex or 1)
    msg:addU8(self.createVoc or 1)
  end
