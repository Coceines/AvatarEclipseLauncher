...
function init()
    ...
    ProtocolGame.registerExtendedOpcode(16, toggle)

    exphWindow = g_ui.loadUI("exph", modules.game_interface.getRightPanel())
    exphButton = modules.client_topmenu.addRightGameToggleButton("exphButton", tr("Experience Stats"), "/images/topbuttons/EXP", toggle)

    exphButton:setOn(exphWindow:isVisible())
    -- Ctrl+H removido (batia com a ajuda do chat). O botao continua abrindo.
    exphWindow:setup()
...
function updateExpHWindow()
    ...
    if _expHistory <= 0 and (0 < expHVar.sessionStart or 0 < _expGained) then
        resetExpH()
        return
    end
    ...
    if 0 < _start and 0 < _expGained then
...
function getExperienceForLevel(lv)
    lv = lv - 1
    return (lv * 50 * lv * lv - lv * 150 * lv + lv * 400) / 3
end
...
function getNumber(msg)
    ...
    return tonumber(string.sub(msg, b, e))
end
...
function updateExpHistory(dif)
    if 0 < dif and expHVar.sessionStart == 0 then
...
function resetExpH()
    ...
    setSkillColor("expph", "#1eff00")
    setSkillValue("expgained", 0)
    setSkillColor("expgained", "#d8ff00")
    setSkillValue("exptolevel", 0)
    setSkillColor("exptolevel", "#ff9600")
    setSkillValue("timetolevel", "00:00")
    setSkillColor("timetolevel", "#ff6767")
...
function expForLevel(level)
    return math.floor(((level * 50 * level * level) / 3 - level * 100 * level + (level * 850) / 3) - 200)
end
...
function setSkillValue(id, value)
    if not exphWindow then return end
    local skill = exphWindow:recursiveGetChildById(id)
    if skill then
        local widget = skill:getChildById("value")
        if widget then
