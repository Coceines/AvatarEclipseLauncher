...
-- Modo "escolha um item" (crosshair): o proximo clique esquerdo no mapa/inventario
-- chama pickCallback(widget, mousePosition). Enquanto ativo o mouse fica preso no
-- mouseGrabberWidget, entao o clique nao anda nem usa nada no jogo.
local pickCallback = nil
local pickCursorName = nil
...
function bindKeys()
  ...
  -- Ctrl+W removido: limpar os textos da tela nao vale um atalho global que
  -- colide com o WASD (quem precisar, o botao esta' no menu do topo).

  -- Space = seleciona o alvo mais proximo (quando chat nao esta focado)
  g_keyboard.bindKeyDown('Space', function()
    -- Checa se um TextEdit tem foco (chat/console)
    local root = g_ui.getRootWidget()
    if root then
      local focused = root:getFocusedChild()
      while focused do
        local cn = focused:getClassName()
        if cn == 'TextEdit' or cn == 'PasswordTextEdit' then return end
    ...
    for _, creature in ipairs(spectators) do
      -- Avatar custom: pet fantasma nunca e alvo (nem do hotkey de mais proximo).
      if creature ~= player and not creature:isLocalPlayer() and not creature:isPet() then
...
function onGameStart()
  ...
  -- Avatar: server manda o effect id dos magic effects com 2 bytes (U16), o
  -- que permite effects custom acima de 255 (ex.: 183+ do Spiritual Rift)
  g_game.enableFeature(GameMagicEffectU16)

  -- Avatar: stack de ate 1000 itens (moedas, Runas, etc)
...
-- Inicia o modo crosshair. `callback` recebe (widget, mousePosition); widget e'
-- nil quando o jogador cancela (botao direito / clique fora). Devolve false se
-- outra coisa ja estiver com o mouse preso.
function startItemPick(callback, cursorName)
  if type(callback) ~= "function" then return false end
  if g_ui.isMouseGrabbed() then return false end

  pickCallback = callback
  pickCursorName = cursorName or 'target'
  mouseGrabberWidget:grabMouse()
  g_mouse.pushCursor(pickCursorName)
  return true
end

function isItemPicking()
  return pickCallback ~= nil
end

function cancelItemPick()
  if not pickCallback then return false end

  local callback = pickCallback
  pickCallback = nil
  if pickCursorName then
    g_mouse.popCursor(pickCursorName)
    pickCursorName = nil
  end
...
function onMouseGrabberRelease(self, mousePosition, mouseButton)
  -- Modo crosshair (market e outros): entrega o widget clicado e solta o mouse.
  if pickCallback then
    ...
    if mouseButton == MouseRightButton then
      callback(nil, mousePosition)
      return true
    end

    local clickedWidget = gameRootPanel:recursiveGetChildByPos(mousePosition, false)
    callback(clickedWidget, mousePosition)
    return true
  end
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
  ...
  if useThing then
    -- Momo Pouch: o server trata como container (slots dinamicos), mesmo que
    -- o DAT do client nao tenha a flag de container. O client id (17437) e
    -- diferente do server id (18435): o server envia o clientId do OTB.
    local isMomoPouch = useThing:isItem() and useThing:getId() == 17437
    if useThing:isContainer() or isMomoPouch then
  ...
  if creatureThing then
    ...
    if creatureThing:isLocalPlayer() then
      ...
      -- Avatar custom: escolher o pet (criatura fantasma). Mesmo menu de contexto
      -- do Set Outfit (botao direito em voce mesmo; no controle classico o menu
      -- abre com Ctrl + botao direito).
      menu:addOption(tr('Pet'), function() g_game.requestPet() end)
    ...
    -- Set Healing Target: so aparece para Water Benders (vocacao 2) em outros players
    if not creatureThing:isLocalPlayer() and creatureThing:isPlayer() then
      local foldsModule = modules.game_folds
      if foldsModule and foldsModule.infos and foldsModule.infos.vocation == 2 then
        local targetName = creatureThing:getName()
        menu:addOption(tr('Set Healing Target'), function()
          G.healTargetName = targetName
          g_settings.set('waterHealTarget', targetName)
          g_settings.save()
          if foldsModule.updateHealTargetTooltip then
            foldsModule.updateHealTargetTooltip()
          end
        end)
      end
    end

    -- Inspect Creature (for non-player creatures)
    if not creatureThing:isLocalPlayer() and not creatureThing:isPlayer() then
      menu:addOption(tr('Inspect Creature'), function()
        local name = creatureThing:getName()
        if name then
          modules.game_inspectcreature.requestMonsterInfo(name)
        end
      end)
    end
  end
    ...
    if not isMenuHookCategoryEmpty(category) then
      ...
      for name,opt in pairs(category) do
        if opt and opt.condition(menuPosition, lookThing, useThing, creatureThing) then
          local label = name
          if opt.name then
            if type(opt.name) == "function" then
              label = opt.name(menuPosition, lookThing, useThing, creatureThing)
            else
              label = opt.name
            end
          end
          menu:addOption(label, function() opt.callback(menuPosition, 
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature)
  -- Avatar custom: o pet fantasma e apenas render visual. Ele nunca participa de
  -- nenhuma acao do mouse (nem look, nem use, nem ataque, nem follow). Filtro
  -- defensivo: mesmo que o pet chegue aqui, vira nil.
  if creatureThing and creatureThing:isCreature() and creatureThing:isPet() then creatureThing = nil end
  if attackCreature and attackCreature:isCreature() and attackCreature:isPet() then attackCreature = nil end
  if lookThing and lookThing:isCreature() and lookThing:isPet() then lookThing = nil end
  if useThing and useThing:isCreature() and useThing:isPet() then useThing = nil end
  ...
  if not modules.client_options.getOption('classicControl') then
    ...
    elseif useThing and keyboardModifiers == KeyboardCtrlModifier and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) then
      -- Momo Pouch: CTRL + botao direito abre a config do pouch ou o menu do item
      if mouseButton == MouseRightButton and modules.game_momo and modules.game_momo.onCtrlRightClick(menuPosition, lookThing, useThing, creatureThing) then
        return true
      end
  ...
  -- classic control
    ...
    elseif lookThing and ((g_mouse.isPressed(MouseLeftButton) and mouseButton == MouseRightButton) or (g_mouse.isPressed(MouseRightButton) and mouseButton == MouseLeftButton)) then
      ...
      return true
    elseif useThing and keyboardModifiers == KeyboardCtrlModifier and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) then
      -- Momo Pouch: CTRL + botao direito -> config do pouch ou menu do item
      if not (mouseButton == MouseRightButton and modules.game_momo and modules.game_momo.onCtrlRightClick(menuPosition, lookThing, useThing, creatureThing)) then
        createThingMenu(menuPosition, lookThing, useThing, creatureThing)
      end
      return true
    elseif attackCreature and g_keyboard.isAltPressed() and (mouseButton == MouseLeftButton or mouseButton == MouseRightButton) then
