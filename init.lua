-- CONFIG
APP_NAME = "otclientv8"  -- nome "compacto": entra no nome do log e na semente da
                         -- criptografia dos dados salvos. Nao define mais a pasta
                         -- no AppData - isso vem do nome do produto em src/main.cpp
                         -- (%APPDATA%\AvatarEclipse).
...
g_app.setName("AvatarEclipse") -- a pasta de configuracao ja' foi decidida no C++
-- CONFIG END
...
if g_app.isMobile() then
  -- only use mobile layout if the client actually ships /layouts/mobile
  if g_resources.directoryExists('/layouts/mobile') then
    layout = "mobile"
  end
elseif settings:exists('layout') then
  layout = settings:getValue('layout')
end
-- nunca passa ao core um layout que o client nao tem (evita o spam
-- "Layour X doesn't exist, using default" a cada startup)
if type(layout) == 'string' then layout = layout:lower() end
if type(layout) ~= 'string' or not g_resources.directoryExists('/layouts/' .. layout) then
  layout = ''
end
