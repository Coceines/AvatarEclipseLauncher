// Rarity: Corrupted (Red) - 4-layer outline with glow
...
void main()
{
    ...
    // Pulsing animation (slightly faster for corrupted feel)
    ...
    // Red color scheme for Corrupted
    vec3 innerColor = vec3(1.0, 0.3, 0.3);   // Bright red (inner)
    vec3 midColor = vec3(0.85, 0.15, 0.15);  // Medium red (middle)
    vec3 outerColor = vec3(0.6, 0.1, 0.1);   // Darker red (outer)
    vec3 glowColor = vec3(1.0, 0.5, 0.5);    // Very bright pink-red (glow)
