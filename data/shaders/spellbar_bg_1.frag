// ============================================================
// SPELLBAR BACKGROUND SHADER - FIRE (Vermelho / Chamas)
// ============================================================
...
void main()
{
    ...
    // Deep dark volcanic base with subtle fiery warmth
    float vCurve = 1.0 - abs(uv.y - 0.5) * 2.0;
    vec3 deepBase = vec3(0.065, 0.028, 0.032) + vec3(0.018, 0.010, 0.012) * vCurve;

    // Fiery heat breathing pulse
    float breath = sin(u_Time * 1.6) * 0.5 + 0.5;

    // Diagonal crimson/flame sheen pass across the bar (4.5s cycle)
    ...
    vec3 sheenCol = vec3(1.00, 0.55, 0.40) * totalSheen * 0.45;

    // Vivid red glow along carved border with flame flow
    ...
    // Fiery inner rim underglow along the top and bottom ridges
