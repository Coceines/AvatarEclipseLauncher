// ============================================================
// TOOLTIP TIER 1 - GREY #9E9E9E (Clean Rounded Borders + Godrays)
// ============================================================
uniform float u_Time;
uniform sampler2D u_Tex0;
varying vec2 v_TexCoord;

const vec3 TIER_COLOR = vec3(0.68, 0.70, 0.75);
...
void main()
{
    ...
    float godrays = rays * godrayFade * 0.40;

    float pulse = sin(u_Time * 2.2) * 0.10 + 0.90;
    float sheen = pow(sin(angle * 3.0 + u_Time * 1.8) * 0.5 + 0.5, 4.0) * borderLine;

    vec3 glow = TIER_COLOR * (borderLine * 1.2 + innerGlow * 0.30 + godrays * 0.65 + sheen * 0.5) * pulse;
    vec3 finalRGB = baseDark + glow;
    finalRGB += vec3(0.4) * sheen * pulse;
